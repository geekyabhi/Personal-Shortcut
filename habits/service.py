...
class HabitsService:
    ...
    def _validate(self, period, year, month, week, day, start="", end=""):
    ...
    def _score_trend(self, period, year, rows, range_start, range_end):
        if period == "daily" or not rows:
            scores = [s for r in rows if (s := self._row_score(r)) is not None]
            return {
        ...
        if period in ("monthly", "weekly"):
            ...
            while d <= range_end:
                ...
                d += timedelta(days=1)
            for row in rows:
                k, s = self._row_date(row), self._row_score(row)
        ...
        if period == "yearly":
            ...
            for row in rows:
                k, s = self._row_date(row)[:7], self._row_score(row)
                if k in buckets and s is not None:
                    buckets[k].append(s)
            return {
    ...
    def _fetch_rows(self, period, year, month, week, day, start, end):
        self._validate(period, year, month, week, day, start, end)
    ...
    def get_chart_data(self, period, year, month, week, day, start, end):
        rows, range_start, range_end = self._fetch_rows(period, year, month, week, day, start, end)
        ...
        return {
            "period": period,
            "total_days": total_days,
            "score_trend": self._score_trend(period, year, rows, range_start, range_end),
    ...
    def check_habits(self, habits_dict):
        ...
        if habits_dict:
            ...
            self.data_layer.patch_page(page_id, patch_props)
